...
anim_plot <- function(data, id = NULL, values = NULL, x_axis = NULL, ...) {

  qid <- enquo(id)
  qvalues <- enquo(values)
  qx_axis <- enquo(x_axis)

  data <- prep_anim(data, id = !!qid, values = !!qvalues, ...)

  hline <- unique(data$pos)

  anim <- data |>
    ggplot() +
    geom_jitter(aes(x = !!qx_axis, y = pos, group = !!qid, color = group), height = 0.1) +
    geom_hline(yintercept = hline, linewidth = 6, alpha = 0.1) +
    scale_x_continuous(breaks = seq(min(year), max(year), 1)) +
    coord_cartesian(xlim = c(min(year), max(year)),
                    clip = 'off') +
